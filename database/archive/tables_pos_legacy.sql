...
-- =============================================================================
-- Roles & permissions (restaurant RBAC)
-- role_type: staff = employees, customer = guest accounts, system = platform
-- =============================================================================
...
CREATE TABLE roles (
    ...
    description TEXT,
    role_type VARCHAR(20) NOT NULL CHECK (role_type IN ('staff', 'customer', 'system')),
...
CREATE TABLE bill_config (
    ...
    currency_symbol VARCHAR(10) DEFAULT '₱',     -- Currency symbol (PHP)
...
-- Optional per-user permission overrides (allow/deny on top of role defaults)
CREATE TABLE user_permissions (
    id SERIAL PRIMARY KEY,
    user_id INT NOT NULL REFERENCES user_login(user_id) ON DELETE CASCADE,
    permission_id INT NOT NULL REFERENCES permissions(permission_id) ON DELETE CASCADE,
    outlet_id INT REFERENCES outlet_configurations(id) ON DELETE CASCADE,
    property_id VARCHAR(50) NOT NULL,
    grant_type VARCHAR(10) NOT NULL DEFAULT 'allow' CHECK (grant_type IN ('allow', 'deny')),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT user_permission_unique UNIQUE (user_id, permission_id, property_id, outlet_id)
);
...
-- user_permissions.property_id aligned in CREATE (VARCHAR(50)); no alter needed for new installs
...
-- Customer role: optional link to guest_record for guest portal logins
ALTER TABLE user_login
ADD COLUMN guest_id INT REFERENCES guest_record(guest_id) ON DELETE SET NULL;
...
CREATE TABLE employees (
    ...
    currency VARCHAR(10) NOT NULL DEFAULT 'PHP', -- Currency for salary payments
...
CREATE TABLE attendance (
    ...
    overtime_hours NUMERIC(5,2) GENERATED ALWAYS AS (
        GREATEST((EXTRACT(EPOCH FROM (shift_end - shift_start)) / 3600) - required_hours, 0)
    ) STORED,
    underworked_hours NUMERIC(5,2) GENERATED ALWAYS AS (
        GREATEST(required_hours - (EXTRACT(EPOCH FROM (shift_end - shift_start)) / 3600), 0)
    ) STORED,
...
CREATE TABLE salaries (
    ...
    final_salary NUMERIC(10,2) GENERATED ALWAYS AS (
        base_salary
        - (base_salary / 30 * absent_days + (underworked_hours * (base_salary / (30 * 8))))
        + (overtime_hours * (base_salary / (30 * 8)))
        + commission_earned
        + tips_earned
        - advance_deduction
    ) STORED,
...
CREATE TABLE expenses (
    ...
    vendor_id INT, -- If related to vendor purchases
...
SELECT 
    ...
    COALESCE(SUM(sm.quantity) FILTER (WHERE sm.change_type = 'Used'), 0) AS total_used,
    COALESCE(SUM(sm.quantity) FILTER (WHERE sm.change_type = 'Wasted'), 0) AS total_wasted,
    ...
    AS closing_stock
FROM ingredients i
LEFT JOIN purchase_items p ON i.ingredient_id = p.ingredient_id
...
AFTER INSERT ON order_items --previously sales 
...
INSERT INTO roles (role_slug, role_name, description, role_type, is_system, display_order) VALUES
('super_admin', 'Super Admin', 'Full platform access across all properties', 'system', TRUE, 1),
('admin', 'Admin', 'Full access within a property (settings, users, reports)', 'staff', TRUE, 2),
('outlet_manager', 'Outlet Manager', 'Manages one outlet: staff, tables, day-end, POS oversight', 'staff', TRUE, 3),
('hr', 'HR', 'Employees, attendance, payroll, benefits', 'staff', TRUE, 4),
('cashier', 'Cashier', 'POS billing, payments, bill generation', 'staff', TRUE, 5),
('waiter', 'Waiter', 'Take orders, assign tables, send KOT', 'staff', TRUE, 6),
('kitchen', 'Kitchen', 'View KOT and order items for preparation', 'staff', TRUE, 7),
('inventory_manager', 'Inventory Manager', 'Stock, ingredients, purchases, vendors', 'staff', TRUE, 8),
('accountant', 'Accountant', 'Expenses, financial reports, bill/payment views', 'staff', TRUE, 9),
('customer', 'Customer', 'Guest self-service: menu, orders, loyalty (linked to guest_record)', 'customer', TRUE, 10);
...
-- Settings & users
('settings.property.manage', 'Manage property settings', 'settings', 'Edit property profile and outlets'),
('settings.outlet.manage', 'Manage outlet configuration', 'settings', 'Outlets, tables, printers, bill/KOT config'),
('users.manage', 'Manage users', 'users', 'Create and deactivate staff logins'),
('users.roles.assign', 'Assign roles to users', 'users', 'Change user role_id'),
('users.permissions.override', 'Override user permissions', 'users', 'Grant or deny extra permissions'),
-- POS & orders
('pos.orders.create', 'Create orders', 'pos', 'Open new orders'),
('pos.orders.update', 'Update orders', 'pos', 'Add/remove items, change table'),
('pos.orders.cancel', 'Cancel orders', 'pos', 'Void orders with reason'),
('pos.orders.modify', 'Modify completed orders', 'pos', 'Post-submit modifications'),
('pos.kot.generate', 'Generate KOT', 'pos', 'Send kitchen order tickets'),
('pos.bills.generate', 'Generate bills', 'pos', 'Create bill from order'),
('pos.bills.void', 'Void bills', 'pos', 'Cancel generated bills'),
('pos.payments.record', 'Record payments', 'pos', 'Cash, card, UPI, split payments'),
('pos.discount.apply', 'Apply discounts', 'pos', 'Manual and promo discounts'),
('pos.tables.manage', 'Manage table status', 'pos', 'Vacant, occupied, reserved'),
('pos.reservations.manage', 'Manage reservations', 'pos', 'Book and update reservations'),
-- Menu & inventory
('menu.items.manage', 'Manage menu items', 'menu', 'Categories, items, pricing'),
('inventory.stock.manage', 'Manage stock', 'inventory', 'Item and ingredient stock'),
('inventory.purchases.manage', 'Manage purchases', 'inventory', 'Vendor purchases and payments'),
-- HR
('hr.employees.manage', 'Manage employees', 'hr', 'Employee CRUD'),
('hr.attendance.manage', 'Manage attendance', 'hr', 'Mark absence, leave, correct punches'),
('hr.attendance.self', 'Self attendance', 'hr', 'Clock in/out for own account'),
('hr.payroll.manage', 'Manage payroll', 'hr', 'Salaries, advances, earnings'),
-- Reports & finance
('reports.sales.view', 'View sales reports', 'reports', 'Revenue and order reports'),
('reports.attendance.view', 'View attendance reports', 'reports', 'Attendance summaries'),
('finance.expenses.manage', 'Manage expenses', 'finance', 'Expenses and approvals'),
-- Guests & loyalty
('guests.manage', 'Manage guests', 'guests', 'Guest records and CRM'),
('loyalty.manage', 'Manage loyalty programs', 'loyalty', 'Points and redemption rules'),
-- Customer (guest portal)
('customer.menu.view', 'View menu', 'customer', 'Browse menu as guest'),
('customer.orders.place', 'Place own orders', 'customer', 'Self-ordering'),
('customer.orders.track', 'Track own orders', 'customer', 'Order status'),
('customer.loyalty.view', 'View loyalty balance', 'customer', 'Points balance and history');
...
JOIN permissions p ON p.permission_key IN (
    'settings.outlet.manage', 'users.manage',
    'pos.orders.create', 'pos.orders.update', 'pos.orders.cancel', 'pos.orders.modify',
    'pos.kot.generate', 'pos.bills.generate', 'pos.bills.void', 'pos.payments.record',
    'pos.discount.apply', 'pos.tables.manage', 'pos.reservations.manage',
    'menu.items.manage', 'inventory.stock.manage',
    'hr.attendance.manage', 'hr.attendance.self',
    'reports.sales.view', 'reports.attendance.view',
    'guests.manage', 'loyalty.manage'
)
WHERE r.role_slug = 'outlet_manager';
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'hr.employees.manage', 'hr.attendance.manage', 'hr.attendance.self',
    'hr.payroll.manage', 'reports.attendance.view', 'reports.sales.view'
)
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'pos.orders.create', 'pos.orders.update', 'pos.bills.generate',
    'pos.payments.record', 'pos.discount.apply', 'pos.tables.manage',
    'guests.manage', 'hr.attendance.self'
)
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'pos.orders.create', 'pos.orders.update', 'pos.kot.generate',
    'pos.tables.manage', 'pos.reservations.manage', 'hr.attendance.self'
)
WHERE r.role_slug = 'waiter';
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'pos.kot.generate', 'pos.orders.update'
)
WHERE r.role_slug = 'kitchen';
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'menu.items.manage', 'inventory.stock.manage', 'inventory.purchases.manage',
    'reports.sales.view', 'hr.attendance.self'
)
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'pos.bills.generate', 'pos.payments.record', 'reports.sales.view',
    'reports.attendance.view', 'finance.expenses.manage', 'hr.payroll.manage'
)
WHERE r.role_slug = 'accountant';
...
FROM roles r
JOIN permissions p ON p.permission_key IN (
    'customer.menu.view', 'customer.orders.place', 'customer.orders.track',
